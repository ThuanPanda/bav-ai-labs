...
Always use the `Button` component from `@/shared/components/ui/button` with explicit `variant` and `size` props. Never override button appearance with raw Tailwind classes — extend the component's CVA definition instead.
...
- **Adding a new style?** Add a new `variant` or `size` to `src/shared/components/ui/button.tsx` CVA, not inline
- **Loading state**: always pass `disabled={isPending}` and render spinner for async actions
