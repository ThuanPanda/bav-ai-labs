---
name: nextjs-colors
description: Enforce color token usage. Always use semantic tokens from globals.css. If a new color is needed, define it in both :root (light) and .dark (dark mode) in src/app/globals.css before using it.
triggers:
  - add color
  - new color
  - color token
...
All colors must come from semantic tokens defined in `src/app/globals.css`. Never hardcode color values. If a required token doesn't exist, define it in both `:root` and `.dark` before using it.
...
1. **Define the token** in `src/app/globals.css` — both `:root` and `.dark`:
...
- **Never use hardcoded hex/rgb/oklch values in JSX/TSX** — only in `globals.css` definitions
- **Never use `dark:` prefix for color overrides** — semantic tokens handle dark mode automatically
- **Never use Tailwind's built-in palette** (`bg-gray-100`, `text-blue-500`, etc.) — always map to a semantic token
- **Always define both `:root` and `.dark`** when adding a new token
